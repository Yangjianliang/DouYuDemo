...
#define DANMU_IP    @"124.95.174.146"
...
- (void)loginToServerWithSocket:(GCDAsyncSocket *)sock {
    ...
    NSData * data = [self msgToServerDataWithContent:[NSString stringWithFormat:@"type@=loginreq/username@=%@/password@=%@/roomid@=%d/", um.isLogin ? um.userInfo.uid : USERNAME, PASSWORD, [_roomId intValue]]];
...
- (void)keepAlive {
    NSData * data = [self msgToServerDataWithContent:[NSString stringWithFormat:@"type@=keeplive/tick@=%ld/",  (NSInteger)[[NSDate date] timeIntervalSince1970]]];
